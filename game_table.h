...
public:
    game_table()= default;
    ~game_table()= default;
    game_table(const game_table& other) : m{other.m}, scor{other.scor} {}
    game_table(const std::array<int,200> m_, int scor_): m{m_}, scor{scor_}{}
    ...
    bool verrify_rotate(std::array <int,200> m){
        std::vector<int> shape=current_pieces[0]->next_rotate();
    ...
    void delete_lines(std::array <int,200> &m){
        for(int i=0;i<20;i++) {
        ...
        while(play) {
            ...
            while (ver) {
                ...
                if(frame_counter%5==0){
                    if(sf::Keyboard::isKeyPressed(sf::Keyboard::Right) && verrify_collision(2,m))  {
